...
// return a default Tab based on the available data
/*
const exampleParameter = {
  data: { 
    snvByGene: {count: 0}, snvByVariant: {count: 0}, 
    cnvByGene: {count: 0}, fusionByGene: {count: 2}, fusion: {count: 0},
  },
  // if GraphQLFieldName and tab Value are the same
  listOfTab: ['snvByGene', 'snvByVariant', 'cnvByGene', 'fusionByGene', 'fusion'],
  listOfTab2: {
    // GraphQLFieldName: TabName
    snvByGene: 'snvByGeneT', snvByVariant: 'snvByVariantT',
    cnvByGene: 'cnvByGeneT', fusionByGene: 'fusionByGeneT', fusion: 'fusionT',
  }
  
  initialDefaultTab: 'snvByGene'
}
*/

export const getDefaultTab = ({ summaryData, widgetTabs, initialDefaultTab }) => {
  let defaultTab = initialDefaultTab;
  if (summaryData) {
    defaultTab = widgetTabs.find(
      dataField =>  summaryData[dataField] && summaryData[dataField].count !== 0
    )
  }
  return defaultTab;
};
